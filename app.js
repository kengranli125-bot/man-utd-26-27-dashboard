...
function sourceFromUrl(url) {
  try {
    ...
    if (host.includes('manutd.com')) return '????';
    return host;
  } catch {
    return '????';
  }
...
function setView(view) {
  ...
  const titles = { overview: '????', fixtures: '?????', squad: '?????', standings: '???', news: '????' };
...
function renderStats() {
  ...
  const items = [['????', s.position ? `${s.position}` : '?', s.position ? '?' : '???'], ['??', s.played ?? 0, '?'], ['??', s.points ?? 0, '?'], ['???', s.goalDifference > 0 ? `+${s.goalDifference}` : (s.goalDifference ?? 0), '?']];
...
function renderNextMatch() {
  ...
  if (!match) { $('#nextMatch').innerHTML = empty('??????', '????????????????????'); return; }
  $('#nextMatch').innerHTML = `<div class="match-card">${teamBlock(match.home)}<div class="match-meta"><span class="competition-badge ${safe(match.competitionGroup || '')}">${safe(match.competition || '???')}</span><time>${fmtDate(match.date, true)}</time><div class="versus">VS</div><span>${safe(match.venue || '????')}</span><small>?????ESPN</small></div>${teamBlock(match.away)}</div>`;
}

function renderForm() {
  const games = state.data.fixtures.filter(f => f.status === 'completed').slice(-5).reverse();
  if (!games.length) { $('#recentForm').innerHTML = empty('??????', '??????????????'); return; }
  $('#recentForm').innerHTML = games.map(g => { const unitedHome = g.home.id === '360'; const us = unitedHome ? g.home.score : g.away.score; const them = unitedHome ? g.away.score : g.home.score; const result = us > them ? ['W','win'] : us === them ? ['D','draw'] : ['L','loss']; const opponent = unitedHome ? g.away : g.home; return `<div class="form-row"><span class="result ${result[1]}">${result[0]}</span><span class="form-opponent"><strong>${safe(opponent.nameZh || opponent.name)}</strong><small>${safe(g.competition || '???')}</small></span><span class="form-score">${us}?${them}</span></div>`; }).join('');
}

function newsCard(article) {
  const image = article.image ? `style="background-image:url('${safe(article.image)}')"` : '';
  return `<a class="news-card" href="${safe(article.url || '#')}" target="_blank" rel="noreferrer"><div class="news-image ${article.image ? '' : 'no-image'}" ${image}></div><div class="news-body"><div class="content-meta"><span class="content-tag">????</span><span>???${safe(itemSource(article))}</span></div><time>${fmtDate(article.published)}</time><h4>${safe(article.titleZh || article.title)}</h4></div></a>`;
}
function renderNews() {
  const news = state.data.news || [];
  const fallback = [{ title: '26/27 ?????????', published: state.data.updatedAt, url: '', image: '' }, { title: '????????????????', published: state.data.updatedAt, url: '', image: '' }, { title: '????????????????', published: state.data.updatedAt, url: '', image: '' }];
...
function renderFixtures() {
  ...
  if (!list.length) { $('#fixtureList').innerHTML = empty(state.filter === 'all' ? '??????' : '???????', '????????????????'); return; }
  const groups = list.reduce((acc, fixture) => { const date = new Date(fixture.date); const key = `${date.getFullYear()}?${date.getMonth()+1}?`; (acc[key] ||= []).push(fixture); return acc; }, {});
  $('#fixtureList').innerHTML = Object.entries(groups).map(([month, fixtures]) => `<section class="fixture-month"><h3>${month}<span>${fixtures.length} ?</span></h3><div>${fixtures.map(g => `<article class="fixture-row"><div class="fixture-date"><strong>${fmtDate(g.date)}</strong>${new Date(g.date).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false})}<span class="competition-badge ${safe(g.competitionGroup)}">${safe(g.competition)}</span></div><div class="fixture-teams"><div class="fixture-team"><img src="${safe(g.home.logo)}" alt="" />${safe(g.home.nameZh || g.home.name)}<span class="score">${g.status === 'completed' ? g.home.score : ''}</span></div><div class="fixture-team"><img src="${safe(g.away.logo)}" alt="" />${safe(g.away.nameZh || g.away.name)}<span class="score">${g.status === 'completed' ? g.away.score : ''}</span></div></div><div class="fixture-venue"><strong>${g.status === 'completed' ? '??' : '??'}</strong>${safe(g.venue || '????')}</div>${renderMatchLineups(g)}${g.status === 'completed' && g.recap ? renderMatchRecap(g.recap) : ''}</article>`).join('')}</div></section>`).join('');
}
...
function renderMatchLineups(fixture) {
  ...
  if (!confirmed.length) {
    ...
    return `<details class="match-lineups pending"><summary>????????? <span>????</span></summary><div class="lineup-wait">???????????????????????????</div></details>`;
  }
  const teamCards = confirmed.map(team => `<section><h5>${safe(team.teamName)}</h5><strong>??</strong><div class="official-player-list">${team.starters.map(player => `<span><i>${safe(player.jersey || '?')}</i>${safe(player.nameZh || player.name)}</span>`).join('')}</div>${team.substitutes.length ? `<strong>??</strong><div class="official-player-list substitutes">${team.substitutes.map(player => `<span><i>${safe(player.jersey || '?')}</i>${safe(player.nameZh || player.name)}</span>`).join('')}</div>` : ''}</section>`).join('');
  return `<details class="match-lineups"><summary>????????? <span>${fixture.status === 'completed' ? '????' : '???'} ?</span></summary><div class="official-lineups">${teamCards}</div></details>`;
}

function renderMatchRecap(recap) {
  const goals = recap.goals?.length ? `<div class="recap-goals"><strong>??</strong>${recap.goals.map(goal => `<span>${safe(goal.nameZh || goal.name)} ${safe(goal.minute)}</span>`).join('')}</div>` : '';
  ...
  const players = recap.playerStats?.length ? `<div class="match-player-data">${recap.playerStats.map(team => `<section><h5>${safe(team.teamName)}????</h5><div class="player-data-scroll"><table><thead><tr><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th><th>??</th></tr></thead><tbody>${team.players.map(player => `<tr><td>${safe(player.nameZh || player.name)}</td><td>${player.starter ? '??' : '??'}</td><td>${safe(player.minutes)}</td><td>${safe(player.goals)}</td><td>${safe(player.assists)}</td><td>${safe(player.shots)}</td><td>${safe(player.shotsOnTarget)}</td><td>${safe(player.saves)}</td><td>${safe(player.yellowCards)}</td><td>${safe(player.redCards)}</td></tr>`).join('')}</tbody></table></div></section>`).join('')}</div>` : '';
  const statsNote = recap.statsAvailable === false ? '<p class="stats-unavailable">???????????????????????????????????</p>' : '';
  return `<details class="match-recap"><summary>????????? <span>?? ?</span></summary><div class="recap-content"><h4>${safe(recap.headline)}</h4><p>${safe(recap.summary)}</p>${goals}${statsNote}${stats}${players}</div></details>`;
}

function renderStandings() {
  const rows = state.data.standings || [];
  if (!rows.length) { $('#standingsBody').innerHTML = `<tr><td colspan="8">${empty('???????', '????????????????')}</td></tr>`; return; }
...
function renderChampionsBoard() {
  ...
  if (!standings.length) { $('#championsBoard').innerHTML = empty('?????????', '?????????????????'); return; }
...
function renderRoster() {
  ...
  if (!players.length) { $('#rosterGrid').innerHTML = empty('????????', '???????????????'); return; }
  $('#rosterGrid').innerHTML = players.map(player => `<article class="roster-card">
    <div class="roster-avatar">${playerAvatar(player)}</div>
    <div class="roster-info"><div class="roster-number">${safe(player.jersey || '?')}</div><div><h4>${safe(player.nameZh || player.name)}</h4><span>${positionZh[player.position] || safe(player.position)} ? ${player.age || '?'}?</span><span class="nationality">${player.flag ? `<img src="${safe(player.flag)}" alt="" />` : ''}${safe(player.nationalityZh || '???')}</span></div></div>
...
function transferCard(item) {
  const status = item.player && item.status ? item.status : '??????';
  const statusType = item.player && item.statusType ? item.statusType : 'report';
  const title = item.player || '??????';
  return `<a class="transfer-item" href="${safe(item.url || '#')}" target="_blank" rel="noreferrer"><span class="transfer-status ${safe(statusType)}">${safe(status)}</span><div><strong>${safe(title)}</strong><p>${safe(item.titleZh || item.title)}</p><small class="transfer-source">???${safe(itemSource(item))}</small></div><time>${fmtDate(item.published)}</time></a>`;
}
function renderTransfers() {
  const transfers = state.data.transfers || [];
  const content = transfers.length ? transfers.map(transferCard).join('') : empty('????????', '??????????????????');
...
function renderJournalists() {
  ...
  $('#journalistsUpdatedAt').textContent = updated ? `?????? ${fmtDate(updated, true)}` : '??????';
  if (!items.length) {
    $('#journalistFeed').innerHTML = empty('????????', '?????????????????????????????????');
    return;
  }
  $('#journalistFeed').innerHTML = items.slice(0, 8).map(item => `<a class="journalist-item" href="${safe(item.url)}" target="_blank" rel="noreferrer"><div class="reporter-mark">${safe((item.reporterZh || item.reporter).slice(0,1))}</div><div class="journalist-copy"><div><strong>${safe(item.reporterZh || item.reporter)}</strong><span>???${safe(item.reporterOutlet || '????')}</span><i class="tier tier-${safe(item.tier)}" title="??????????????">T${safe(item.tier)} ??</i></div><p>${safe(item.titleZh || item.title)}</p><small>?????${safe(item.source || '???')} ? ???${safe(sourceFromUrl(item.url))} ? ${fmtDate(item.published, true)}</small></div><b aria-hidden="true">?</b></a>`).join('');
}
...
function renderStaff() {
  ...
  $('#staffGrid').innerHTML = `${confirmed}<article class="staff-card pending-staff"><div class="staff-placeholder">+</div><div><span class="staff-status pending">????</span><h4>??????</h4><p>??????????????</p><small>???????????</small></div></article>`;
}

function renderLineup() {
  const prediction = state.data.predictions?.[state.lineup];
  if (!prediction?.players?.length) { $('#lineupPitch').innerHTML = empty('???????', '???????????????'); return; }
...
async function loadLatestData({ initial = false } = {}) {
  try {
    ...
    if (initial) $('#seasonStatus').textContent = '??????????????';
  }
}
